    ...
    try {
      ...
      const options = {
        ...
        prefill: {
          ...
          email: user.email || "user@schoolslip.com",
        },
        notes: {
          userId: user.uid,
          packageId: packageId,
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-xl text-slate-800 tracking-tight">SchoolSlip</span>
          </div>
      ...
      {isAuthModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-fade-in">
          <div className="bg-white/95 dark:bg-slate-900/95 border border-slate-200/80 dark:border-slate-800 rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl relative flex flex-col gap-6 animate-scale-up">
            
            {/* Close Button */}
            <button 
              onClick={() => { if (!loadingAuthSubmit) setIsAuthModalOpen(false); }}
              disabled={loadingAuthSubmit}
              className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors disabled:opacity-50"
            >
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-slate-400 mt-1">
                {authMode === 'login' ? 'Sign in to access your print credits' : 'Register to unlock unlimited slipping downloads'}
              </p>
            </div>

            {/* Error Notification Badge */}
            {authError && (
              <div className="p-3.5 rounded-xl bg-rose-50 border border-rose-100 text-rose-600 text-xs font-semibold flex items-center gap-2 animate-shake">
            ...
            <button
              ...
              className="w-full py-2.5 rounded-xl border border-slate-200 hover:border-slate-300 dark:border-slate-800 dark:hover:border-slate-700 bg-white dark:bg-slate-900 hover:bg-slate-50 text-slate-700 dark:text-white text-sm font-extrabold shadow-sm flex items-center justify-center gap-2.5 transition-all duration-300 hover:scale-[1.01] active:scale-[0.99] cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <form onSubmit={handleAuthSubmit} className="flex flex-col gap-4">
              ...
              {authMode === 'signup' && (
                <div className="flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    className="px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white/50 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none text-sm transition-all text-slate-800 dark:text-white font-medium"
                  />
                </div>
              )}
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  type="email"
                  required
                  placeholder="parent@schoolslip.com"
                  value={authEmail}
              ...
              <button
                type="submit"
                disabled={loadingAuthSubmit}
                className="w-full py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white text-sm font-extrabold hover:scale-[1.01] active:scale-[0.99] transition-all shadow-md shadow-indigo-100 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed mt-2 cursor-pointer"
              >
                {loadingAuthSubmit ? (
                  <>
